...
object CourseLearnStreaming {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //解析json数据
    val dsStream: DStream[LearnModel] = stream.mapPartitions(partitions => {
      partitions.map(item => {
        val json = item.value()
        val jsonObject = ParseJsonData.getJsonData(json)
        val userId = jsonObject.getIntValue("uid") //用户ID
        val cwareid = jsonObject.getIntValue("cwareid") //课件ID
        val videoId = jsonObject.getIntValue("videoid") //视频ID
        val chapterId = jsonObject.getIntValue("chapterid") //章节ID
        val edutypeId = jsonObject.getIntValue("edutypeid") //辅导ID
        val subjectId = jsonObject.getIntValue("subjectid") //题目ID
    ...
    dsStream.foreachRDD(rdd => {
      ...
      }).reduceByKey(_ + _)  //  （章节ID,当前批次视频播放总时长）
        //1,1   2,1   1,1   >>   1,2   2,1
  ...
  def calcVideoTime(key: String, iters: Iterable[LearnModel], sqlProxy: SqlProxy, client: Connection) = {
    ...
    val cwareId = keys(1).toInt   //课件id
    val videoId = keys(2).toInt   //视频id

    //查询历史数据
    // interval_history: ps（视频开始区间）-pe（视频结束区间）
    ...
    //iters : LearnModel(userId, cwareid, videoId, chapterId, edutypeId, subjectId, sourceType, speed, ts, te, ps, pe)
    ...
    learnList.foreach(item => {
      if ("".equals(interval_history)) {
        ...
        effective_duration_sum += effective_duration.toLong  //有效总时长 += 有效时长
        cumulative_duration_sum += effective_duration.toLong   //播放总时长 += 播放总时长
        complete_duration_sum += complete_duration  //完成总时长 += 完成时长

        interval_history = play_interval
      } else {
        //有历史区间进行对比

        val interval_arry = interval_history.split(",")
          .sortBy(a => (a.split("-")(0).toInt, a.split("-")(1).toInt))  // ps（视频开始区间），pe（视频结束区间）
        val tuple: (Int, String) = getEffectiveInterval(interval_arry, item.ps, item.pe)
        ...
        interval_history = tuple._2   // ps（视频开始区间），pe（视频结束区间）

        effective_duration_sum += effective_duration.toLong  //计算有效时长
        complete_duration_sum += complete_duration  //获取实际有效完成时长
        cumulative_duration_sum += cumulative_duration.toLong   //累计时长
      }
  ...
  /**
    * 计算有效区间   ()
    * 获取实际有效完成时长  和   ps（视频开始区间），pe（视频结束区间）
    *
    * @param array
    * @param start
    * @param end
